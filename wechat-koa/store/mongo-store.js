 ...
 * @private
 */
MongoStore.prototype._connect = Promise.coroutine(function*(collectionName) {
  // already done?
  ...
  var _collectionName = collectionName || defaultOptions.collection || options.collection;
  try {
    debug('open collection %s', collectionName);
    collection = yield Promise.promisify(openedDb.collection, openedDb)(_collectionName);
  } catch (err) {
    throw new Error('Error opening collection ' + _collectionName + ': ' + err.message);
  }
